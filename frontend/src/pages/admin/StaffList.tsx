...
const StaffList: React.FC = () => {
  ...
  useEffect(() => {
    const filterStaff = async () => {
      const result = await StaffService.searchStaff(filters)
  ...
  const handleDeactivateStaff = async (staffId: string) => {
    // Lấy current user từ localStorage để kiểm tra
  ...
  const exportAllData = () => {
    const csvContent = [
      ['Tên', 'Email', 'Mã NV', 'Số điện thoại', 'Địa chỉ', 'Vai trò', 'Trạng thái', 'Ngày tạo'],
      ...staff.map(staffMember => [
        staffMember.fullName,
        staffMember.email,
        staffMember.customerCode,
        staffMember.phoneNumber || '',
        staffMember.address || '',
        staffMember.roles.join(', '),
        staffMember.isActive ? 'Hoạt động' : 'Vô hiệu hóa',
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600 mt-2">Quản lý thông tin và quyền hạn của nhân viên</p>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.newStaffThisMonth}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <select
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                value={filters.role || ''}
                onChange={(e) => setFilters({ ...filters, role: e.target.value as 'Admin' | 'Staff' || undefined })}
              >
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            <h2 className="text-lg font-semibold text-gray-900">
              Danh sách nhân viên ({filteredStaff.length}/{staff.length})
            </h2>
          </div>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {currentStaff.map((staffMember) => (
                  <tr key={staffMember.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm font-mono text-purple-600">{staffMember.customerCode}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="text-sm text-gray-900">{staffMember.phoneNumber || 'Chưa có'}</div>
                      <div className="text-sm text-gray-500">{staffMember.address || 'Chưa có địa chỉ'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      {getRoleBadge(staffMember.roles)}
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex space-x-2">
                        <Link
                          to={`/admin/staff/${staffMember.customerCode}`}
              ...
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm text-gray-700">
                    Hiển thị{' '}
                    <span className="font-medium">{indexOfFirstStaff + 1}</span>
                    {' '}đến{' '}
